// Lobby: room lifecycle (create/join/leave), socket attachment & reconnect,
// match start, and prompt-answer relay. Extracted from index.js so the HTTP
// wiring and the multiplayer state machine can evolve separately.
...
export function attachLobby(io) {
  // code -> { code, mode, playerCount, deckMode, players: [{token, name, seat,
  //           socket, userId, deckCards}], match, started, idleSince }
  const rooms = new Map();
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("createGame", ({ name, mode, playerCount, deckMode, deckId }, cb) => {
      ...
      if (rooms.size >= config.maxRooms) {
        return cb?.({ ok: false, error: "The server is full right now — try again later." });
      }
  ...
  // Reap rooms that are finished or fully abandoned so a long-running server
  // doesn't accumulate them. A room is "idle" once its match is over or every
  // player has disconnected; it is deleted after config.roomReapMinutes idle
  // (reconnect tokens stop working then, which is the intended cleanup).
  const reaper = setInterval(() => {
    const now = Date.now();
    let changed = false;
    for (const room of rooms.values()) {
      const over = room.match?.matchWinner != null;
      const abandoned = room.players.every((p) => !p.socket);
      if (over || abandoned) {
        room.idleSince ??= now;
        if (now - room.idleSince >= config.roomReapMinutes * 60 * 1000) {
